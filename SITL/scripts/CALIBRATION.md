...
Data lives in `SITL/data/<TARGET>/` with a README describing each
capture; the model json goes in `SITL/models/`.
...
Convert a capture's raw dump with
    ...
    SITL/sitl_params.py dump sitl_eeprom.bin > sitl.param
...
- current calibration if the sense scale is unknown: steady
  ...
  MILLIVOLT_PER_AMP and CURRENT_OFFSET.
