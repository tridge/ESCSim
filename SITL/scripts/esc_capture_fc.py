...
SAFETY: strap the motor down carefully if props are on, and keep a way
to cut power - the battery connector or a switch - within reach.
Betaflight holds the last motor-test value it was given and applies no
failsafe to it. This tool aborts on over-current, over-temperature,
telemetry loss or arming, and commands stop when it exits, but it
cannot do so if the USB link dies or it is killed outright. Betaflight
also serves its last decoded telemetry indefinitely once the ESC's
replies stop, which is only detectable here when the throttle is
changing.
...
class BetaflightBackend(object):
    ...
    def wait_ready(self):
        ...
        self._preflight()
        print('Motor secured? starting in %.0fs (Ctrl-C to abort)' %
              self.args.countdown)
    ...
    def _handle_telemetry(self, at, data):
        ...
        if fp != self.last_fingerprint:
            ...
            if self.ever_turned:
                ...
                # nothing has ever turned, so this is a motor that is
                # not running rather than replies that stopped arriving
                self.abort('the motor did not turn in the %.1fs after '
                           'throttle was commanded (rpm still 0, replies '
                           'still arriving). Check the ESC has battery '
                           'power and that the motor is wired to output %u'
                           % (self.args.freeze_timeout, self.args.motor))
    ...
    def _preflight(self):
        ...
        # an ESC whose MCU is powered from the FC's 5V talks BDShot and
        # answers 4-way perfectly while its power stage has no voltage,
        # so the motor simply never turns. Say so before the capture
        # rather than after a throttle timeout
        try:
            bat = msp.parse_battery_state(
                self.port.request(msp.MSP_BATTERY_STATE))
        except Exception:
            bat = {}
        if bat and not bat['present']:
            print('WARNING: the flight controller reports no battery '
                  '(%.2fV) - if the ESC has no pack voltage the motor '
                  'cannot turn, whatever the telemetry says' % bat['volts'])
        elif bat:
            print('battery: %.2fV, %u cells (%s)'
                  % (bat['volts'], bat['cells'], bat['state_name']))
