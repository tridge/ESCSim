...
def main():
    ...
    ap.add_argument('--sitl', nargs='?', default=None, const='',
                    help='also test capture and abort against a live '
                         'simulated ESC (default: the SITL built in the '
                         'AM32 checkout)')
    args = ap.parse_args()
    if args.sitl is not None:
        args.sitl = am32_paths.sitl_binary(args.sitl or None)
