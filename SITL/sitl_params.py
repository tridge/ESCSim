...
def _version_defines():
    '''VERSION_MAJOR/MINOR and EEPROM_VERSION from the firmware's
    Inc/version.h so the defaults below track it instead of being
    duplicated. Falls back to the listed values when no firmware
    checkout is available (a frozen GUI build)'''
    d = {'VERSION_MAJOR': 2, 'VERSION_MINOR': 20, 'EEPROM_VERSION': 3}
    root = am32_paths.am32_root(required=False)
    if root is None:
        return d
    path = os.path.join(root, 'Inc', 'version.h')
    try:
...
def _firmware_defaults():
    ...
    goes stale here'''
    path = os.path.join(am32_paths.am32_root(), 'Src', 'DroneCAN',
                        'DroneCAN.c')
    m = re.search(r'default_settings\[\]\s*=\s*\{(.*?)\}\s*;',
                  open(path).read(), re.S)
    if not m:
        raise ValueError('no default_settings[] in %s' % path)
    return bytes(int(v, 16) for v in re.findall(r'0x([0-9a-fA-F]+)',
                                                m.group(1)))
...
def base_image():
    ...
    from Inc/version.h so they track the firmware'''
...
def format_param_file(image, header=''):
    ...
    lines += [
        ...
        '# SITL/sitl_params.py, never committed as a binary.',
        '']
    if unmapped:
