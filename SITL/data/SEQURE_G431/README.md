...
`SITL/scripts/esc_measure.py`; compare with `SITL/scripts/esc_analyze.py`.

The matching SITL model is `SITL/models/sequre_gt2215.json` (v9).
...
- `sitl_sweep_v9.jsonl`, `sitl_chirp_120s_v9.jsonl`,
  ...
  `python3 SITL/scripts/esc_plot.py SITL/data/SEQURE_G431/square1.jsonl SITL/data/SEQURE_G431/sitl_square_v9.jsonl`
...
- **current sense**: the amp is bidirectional, biased near mid-rail
  ...
  wrapped negative and clamped to 0. The recorded current captures used
  an experimental bias-compensation build that is not part of this tree.
  **Calibrated 2026-07-19 by finger-load test** (`finger_load1.jsonl`,
  ...
  ~2.0 mV/A, though the SEQURE_G431 targets retain the configured
  MILLIVOLT_PER_AMP value of 5. Resolution ~0.4 A/count (large ESC).
