...
def _version_defines():
    ...
    d = {"VERSION_MAJOR": 2, "VERSION_MINOR": 21, "EEPROM_VERSION": 4}
    path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "..", "..", "Inc", "version.h"
    )
    try:
...
CALC = {
    ...
    "DIR_REVERSED": _onoff,
    "BI_DIRECTIONAL": _onoff,
    "USE_SINE_START": _onoff,
    "COMP_PWM": _onoff,
    "VARIABLE_PWM": _onoff,
    "STUCK_ROTOR_PROTECTION": _onoff,
    "BRAKE_ON_STOP": _onoff,
    "STALL_PROTECTION": _onoff,
    "AUTO_ADVANCE": _onoff,
    "RC_CAR_REVERSE": _onoff,
    "USE_HALL_SENSORS": _onoff,
    "REQUIRE_ARMING": _onoff,
    "REQUIRE_ZERO_THROTTLE": _onoff,
    "TERM_ENABLE": _onoff,
    "DISABLE_STICK_CALIBRATION": _onoff,
...
def _firmware_defaults():
    """the factory settings the firmware itself carries, from
    default_settings[] in Src/DroneCAN/DroneCAN.c, so a generated image
    starts where a factory flashed ESC does instead of from a copy that
    goes stale here"""
    # AM32 2.21's factory image. Firmware packs can carry a matching EEPROM
    # image for older releases; this built-in is the network-free default.
    return bytes.fromhex(
        "0103010123a004000a64003202003531200000000101011a1864370e00000500"
        "80808032003200000f0a0a8d66060100"
    )
...
def format_param_file(image, header=""):
    ...
    lines += [
        ...
        "# Src/DroneCAN/DroneCAN.c; the image is generated by",
        "# Mcu/SITL/sitl_params.py, never committed as a binary.",
        "",
    ]
    if unmapped:
        lines += [
            "# NOTE: unnamed bytes also differ from the defaults at "
    ...
    for off, name, _mn, _mx, _dflt, _help in PARAMS:
        ...
        lines.append(
            "%-24s %3d%s" % (name, overrides[name], "   # %s" % calc if calc else "")
        )
    return "\n".join(lines) + "\n"
