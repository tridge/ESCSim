...
from .backend import AudioStream, ToneStream
        ...
        class SineDevice(QIODevice):
            ...
            def readData(self, maxlen):
                ...
                for _ in range(n):
                    ...
                    out += struct.pack(
                        "<h", int(32767 * self.gain * math.sin(self.phase))
                    )
                return bytes(out)
...
def cmd_play(stream, args):
    ...
    if not synth.active:
        ...
        return 1
    print(
        "playing %s from udp %s:%u, ctrl-c to stop"
        % ("physics audio" if args.physics else "tones", stream.addr[0], stream.addr[1])
    )
...
def render_physics_wav(batches, path):
    ...
    frames = b"".join(
        struct.pack("<h", int(max(-32767, min(32767, v * scale)))) for v in vals
    )
    with wave.open(path, "wb") as w:
        w.setnchannels(1)
        w.setsampwidth(2)
...
def cmd_wav(stream, args):
    ...
    print(
        "wrote %s: %d events, %.2fs" % (args.out, len(events), nsamples / SAMPLE_RATE)
    )
    return 0
...
def match_notes(notes, expected, freq_tol, dur_tol):
    ...
    def match(n, e):
        return abs(n[0] - e[0]) <= e[0] * freq_tol and abs(n[1] - e[1]) <= dur_tol
...
def cmd_assert(stream, args):
    ...
    if found:
        for (freq, dur, amp), (efreq, edur) in zip(found, expected):
            print("PASS: %.1fHz %.3fs (want %.1fHz %.3fs)" % (freq, dur, efreq, edur))
        return 0
...
def main():
    ...
    ap.add_argument(
        "--port", type=int, default=57734, help="SITL state port (default 57734)"
    )
    ...
    p.add_argument(
        "--notes", required=True, help="freq_hz:duration_s,... e.g. 428.6:0.2,585.4:0.2"
    )
    p.add_argument(
        "--freq-tol",
        type=float,
