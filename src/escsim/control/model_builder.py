...
def build_model(spec, notes=None):
    ...
    if r_ll:
        ...
        note(
            "resistance %.4f ohm/phase (%.0f mohm line-to-line / 2)"
            % (resistance, r_ll)
        )
    else:
        resistance = 0.06
        note("resistance %.4f ohm/phase (default, no spec value)" % resistance)
    ...
    note(
        "inductance %.2e H (2.2e6 * stator_vol %.2e / kv^2)" % (inductance, stator_vol)
    )
    ...
    if weight_g:
        ...
        note(
            "rotor inertia %.2e kg m^2 (0.55 * %.0fg at r %.1fmm)"
            % (inertia_motor, weight_g, r_eff * 1e3)
        )
    else:
        inertia_motor = 0.3 * stator_vol * 1e3 * (r_stator**2) * 20
        note("rotor inertia %.2e kg m^2 (size estimate, no weight)" % inertia_motor)
    ...
    if idle:
        ...
        note(
            "damping %.2e N m s/rad, static_friction %.2e N m "
    ...
    if not voltage and cells:
        voltage = 3.8 * cells
        note(
            "pack voltage %.1fV (%uS at 3.8V, override with --voltage)"
            % (voltage, cells)
        )
    elif not voltage:
        voltage = 12.0
    ...
    note(
        "sink %s: resistance %.2f ohm, current_max %.2f A"
...
def main():
    ...
    ap.add_argument(
        "--size", required=True, help="motor size code, e.g. 2216 (22mm dia, 16mm high)"
    )
    ap.add_argument(
        "--poles", type=int, required=True, help="magnet pole count (e.g. 14)"
    )
    ...
    spec = {
        ...
        "esc_current_a": args.esc_current,
