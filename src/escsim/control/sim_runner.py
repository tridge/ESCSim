...
def bundled_sitl():
    ...
    base = _resource_dir()
    for cand in (
        os.path.join(base, "sitl", _exe("AM32_SITL_CAN")),
...
def bundled_eeprom():
    ...
    if not os.path.isfile(out) or os.path.getmtime(out) < os.path.getmtime(params):
...
def _win_job_start(pid):
    ...
    if (
        not k32.SetInformationJobObject(
            hjob, EXTENDED_LIMIT_CLASS, ctypes.byref(info), ctypes.sizeof(info)
        )
        or not hproc
        or not k32.AssignProcessToJobObject(hjob, hproc)
    ):
        if hproc:
...
def _win_job_active(job):
    ...
    if not k32.QueryInformationJobObject(
        hjob, ACCOUNTING_CLASS, ctypes.byref(info), ctypes.sizeof(info), None
    ):
        return False
...
class SimRunner(object):
    ...
    def _pump(self):
        ...
        if self.proc is proc and self._requested_stop is not proc and status != 0:
