...
try:
    ...
    dronecan.app.node_monitor.NodeMonitor._on_info_response = _guarded_on_info
    ...
    if _busy_poll:
        ...
        def _blocking_receive(self, timeout=None):
            deadline = None if timeout is None else time.time() + max(timeout, 0.001)
            while True:
                try:
                    if deadline is None:
                        ...
                        if remaining <= 0:
                            return
                        frame = self.rx_queue.get(block=True, timeout=remaining)
                except queue.Empty:
                    if deadline is None:
                        continue
                    return
...
class DshotPanel(object):
            ...
            while now >= next_send and burst < 10:
                ...
                if cmd is not None:
                    self.port.send_dshot(
                        cmd, ptype=self.ptype, telem=True, bidir=self.bidir
                    )
    ...
    def _edt_maintain(self, now):
        ...
        if self.cmd_queue.qsize() < EDT_CMD_QUEUE:
            ...
            self.send_command(
                sd.DSHOT_CMD_EDT_ENABLE if want_enable else sd.DSHOT_CMD_EDT_DISABLE,
                count=EDT_CMD_QUEUE,
            )
            status = (
                "EDT enable sent, waiting for EDT frames (arms after >1.5s at zero)"
                if want_enable
                else "EDT disable sent"
            )
...
class CanPanel(object):
    ...
    def _handle_param(self, node):
        ...
        self.param_result.put(
            "%s=%d saved, node %d restarted" % (name, int(value), target)
        )
...
class EepromClient(object):
    ...
    def set(self, offset, data):
        ...
        try:
            pkt = struct.pack(
                "<HBBHH", self.MAGIC_CMD, 6, 0, int(offset), len(data)
            ) + bytes(data)
    ...
    def _reader(self):
        while self.running:
            ...
            if magic == self.MAGIC_INFO and len(d) >= 20:
                pc, app_base, armed_count, loop_hz = struct.unpack_from("<IIII", d, 4)
...
class AudioStream(object):
    ...
    def _reader(self):
        while self.running:
            ...
            if magic != self.MAGIC_AUDIO or ver != 1 or len(d) < 16 + 4 * count:
                continue
