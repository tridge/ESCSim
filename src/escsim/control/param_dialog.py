...
class ParamDialog(QDialog):
    def __init__(self, eeprom_client, parent=None):
        ...
        self.table.setHorizontalHeaderLabels(
            ["parameter", "raw value", "", "value", "range", "default", "description"]
        )
    ...
    def build_rows(self):
        for row, (off, name, lo, hi, dflt, helptext) in enumerate(sitl_params.PARAMS):
    ...
    def recolour(self):
        ...
        for row, (off, name, lo, hi, dflt, _h) in enumerate(sitl_params.PARAMS):
            spin = self.spins[name]
            item = self.table.item(row, 0)
            self.table.item(row, 3).setText(sitl_params.calc_value(name, spin.value()))
            ...
            btn.setStyleSheet("" if at_default else "color: #1560c0; font-weight: bold")
            btn.setToolTip(
                "at the default (%d)" % dval
                if at_default
                else "not at the default: reset to %d%s"
            ...
            if name in checks:
                ...
                if spin.value() != want:
                    colour = MISMATCH_BG
                    tips.append(
                        "does not match the simulated motor "
                        "(needs %d)\n%s" % (want, why)
                    )
            if stored is not None and spin.value() != stored:
                colour = colour or CHANGED_BG
                tips.append("changed from %d, not yet applied" % stored)
            item.setBackground(QBrush(colour) if colour else QBrush(Qt.transparent))
            item.setToolTip(
                "eeprom offset %d\n%s" % (off, "\n".join(tips))
                if tips
                else "eeprom offset %d" % off
            )
