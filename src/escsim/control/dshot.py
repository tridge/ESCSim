...
class InputPort(object):
    ...
    def send_dshot(
        self, value11, ptype=TYPE_DSHOT300, telem=False, bidir=False, corrupt=False
    ):
    ...
    def send_serial(self, payload, idle_high=True, gap=False):
        ...
        for ofs in range(0, len(payload), SERIAL_MAX):
            self.sock.sendto(
                pack_serial(payload[ofs : ofs + SERIAL_MAX], flags), self.addr
            )
            flags &= ~FLAG_GAP
        self.sent_count += 1
...
def set_dronecan_param(
    name, value, uri="mcast:0", timeout=10.0, save=True, restart=True, node_id=None
):
